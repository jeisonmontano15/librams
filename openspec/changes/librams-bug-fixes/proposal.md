...
1. **Checkout is broken against the committed schema.** `LoanRepository.CheckOutAsync`
   inserts into a `loans.user_email` column that `schema.sql` never creates. Anyone
   provisioning the database from the committed schema gets a `42703: column
   "user_email" does not exist` on every checkout — a core feature failing on a clean
   setup. The deployed database has the column, so this is schema drift: the committed
   schema no longer reproduces production.
