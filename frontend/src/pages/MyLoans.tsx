...
export function MyLoans() {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-3xl">
      <h1 className="font-serif text-2xl sm:text-3xl text-ink-900 mb-8">My loans</h1>
        ...
        {historyLoading ? (
          ...
          <div className="divide-y divide-ink-100 border border-ink-100 rounded-sm overflow-hidden">
            {history?.filter(l => l.status === 'returned').map(loan => (
              <div key={loan.id} className="flex items-center gap-3 sm:gap-4 px-3 sm:px-4 py-3 bg-paper-light hover:bg-ink-50 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-ink-400 font-sans truncate">{loan.book?.author}</p>
                </div>
...
function LoanRow({ loan, onCheckIn, checkingIn }: { loan: Loan; onCheckIn: (l: Loan) => void; checkingIn: boolean }) {
  ...
  return (
    <div className={clsx(
      'flex items-start sm:items-center gap-3 sm:gap-4 p-3 sm:p-4 border rounded-sm',
      ...
      {/* Info — the Return button drops below this block on narrow screens, so the title
          and due date keep the full row width instead of competing with the control. */}
      <div className="flex-1 min-w-0">
        ...
        <div className={clsx('flex items-center gap-1 mt-1 text-xs font-sans',
          ...
          {overdue   ? <AlertCircle size={11} className="flex-shrink-0" /> : <Clock size={11} className="flex-shrink-0" />}
          {overdue
        ...
        {/* Mobile-only action, inline under the details */}
        <button
          onClick={() => onCheckIn(loan)}
          disabled={checkingIn}
          className="sm:hidden mt-3 flex items-center gap-1.5 px-3 py-2 border border-ink-300 text-ink-700 text-xs font-sans rounded-sm hover:bg-ink-100 transition-colors disabled:opacity-60"
        >
          <RotateCcw size={12} />
          Return
        </button>
      ...
      {/* Check in button — `sm` and up, at the end of the row */}
      <button
        onClick={() => onCheckIn(loan)}
        disabled={checkingIn}
        className="hidden sm:flex items-center gap-1.5 px-3 py-2 border border-ink-300 text-ink-700 text-xs font-sans rounded-sm hover:bg-ink-100 transition-colors disabled:opacity-60 flex-shrink-0"
      >
        <RotateCcw size={12} />
        Return
      </button>
